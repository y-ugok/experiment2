...
window.onload = function () {
  savedColor = storage.getItem("themeColor") || "#FFC7BF";
};
const open =
...
function loadList() {
 ...
 `;
    ...
    if (editButton) {
      applyThemeColorToButton(editButton); // 追加した関数を使ってボタンの色を設定
    }
    applyThemeColor(savedColor);
  });
  function applyThemeColorToButton(button) {
    const savedColor = localStorage.getItem("themeColor") || "#FFBDB1";
    button.style.backgroundColor = savedColor; // 'edit'ボタンにカラーを設定
  }
...
function addDotsFunctionality() {
  const dotIcons = document.querySelectorAll("#edit");
...
updateButton.addEventListener("click", async () => {
  ...
  if (oldText === listText.value && oldIcon === newItem.icon) {
    alert("アイコンの種類とリストアイテムの内容が同じです");
    return; // 関数を終了する
  }

  // firebaseから削除
  let storedItems = JSON.parse(sessionStorage.getItem("self-list")) || [];
  const delItem = storedItems.find((item) => item.text == oldText);
  await firestore.deleteData("self-list", delItem.docID);
  const docID = await firestore.addData("self-list", newItem);
  newItem.docID = docID;
